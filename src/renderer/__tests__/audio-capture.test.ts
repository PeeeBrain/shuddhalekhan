...
describe('warm audio stream', () => {
  ...
  it('throttles audio-level-changed to a bounded UI animation rate', async () => {
    installAudioMocks();
    const mod = await importWarm();
    mod.setSelectedDeviceId('mic-1');
    ...
    try {
      ...
      const levelCalls = (send as ReturnType<typeof mock>).mock.calls.filter(
        ([channel]: [string, ...unknown[]]) => channel === 'audio-level-changed'
      );
  ...
  it('does not emit any duration IPC because the recording pill owns elapsed time locally', async () => {
    installAudioMocks();
    const mod = await importWarm();
    mod.setSelectedDeviceId('mic-1');
    ...
    const durationCalls = (send as ReturnType<typeof mock>).mock.calls.filter(
      ([channel]: [string, ...unknown[]]) => channel === 'audio-duration-changed'
    );
